...
public class Solution2 {
    ...
    public static Map<Character, List<Integer>> charTocIdxMao(char[] c) {
    ...
    public String longestPalindrome(String s) {
        ...
        char[] c2;
        Map<Character, List<Integer>> cIdxMap = charTocIdxMao(c);
...
//        int cLength = c.length;
        ...
        for (char eachC : cIdxMap.keySet()){
            List<Integer> idxsOfEachC = cIdxMap.get(eachC);
            tmpIdxAmount = idxsOfEachC.size();
            startIdx = idxsOfEachC.get(0);
            endIdx = tmpIdxAmount == 1 ? startIdx : idxsOfEachC.get(tmpIdxAmount - 1);
            if (tmpIdxAmount == 1) {
...
//                    if (idxsOfEachC[i] == endIdx) continue;
                    for (int j = tmpIdxAmount - 1 ; j > 0 ; j--){
                        if(((idxsOfEachC.get(j) - idxsOfEachC.get(i) + 1) > longestLength) && isPalindrome(c, idxsOfEachC.get(i), idxsOfEachC.get(j))) {
                            longestStartIndex = idxsOfEachC.get(i);
                            longestEndIndex = idxsOfEachC.get(j);
...
//        System.out.println(isPalindrome("abvzsvba".toCharArray(), 0, 7));
        ...
        System.out.println(new Solution2().longestPalindrome(ss));
        System.out.println(new Solution2().longestPalindrome(sss));
